...
var resetImg = function*() {
        ...
        if (convertPath) {
            ...
            var cachePath = path.join(global.config.convert_data_path, 'thumbnail')
...
var resetDoc = function*() {
        ...
        if (docPath && gsPath) {
            ...
            if (status) {
                //cache file save path
                var cachePath = path.join(global.config.convert_data_path, 'doc')
                if (!fs.existsSync(cachePath)) {
                    mkdirp.sync(cachePath)
                }
...
var resetVideo = function*() {
        ...
        if (videoPath) {
            ...
            if (status) {
                //cache file save path
                var cachePath = path.join(global.config.convert_data_path, 'video')
                if (!fs.existsSync(cachePath)) {
                    mkdirp.sync(cachePath)
                }
