    ...
    let ImageConvert = function(convertPath, identifyPath, cachePath) {
         ...
         * @api public
         */
    ImageConvert.prototype.getThumbnailPath = function*(key, w, h) {
            //default 640x640
            if (isNaN(w)) {
                w = 640
            }
            let thumbnailPath = path.join(this.cachePath, key + '-' + w + 'x' + h)
            ...
            if (!fileUtils.exists(thumbnailPath)) {
                ...
                if (sourcePath) {
                    thumbnailPath = yield this.createThumbnail(sourcePath, key, w, h)
                }
            }
            if (fileUtils.exists(thumbnailPath)) {
         ...
         * @api public
         */
    ImageConvert.prototype.createThumbnail = function(sourcePath, key, width,height) {
        let self = this
        return function(done) {
            let aimPath = path.join(self.cachePath, key + '-' + width + 'x' + height + '.png')
