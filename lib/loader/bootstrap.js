...
let logFolderPath = path.join(rootPath, 'logs')
     ...
     * @api public
     */
let __exists = function(filePath) {
        try {
    ...
    /**
     * set app config save path     
     * @api private
     */
let __setConfigRootPath = function() {
        rootPath = path.join(__dirname, '..', '..')
        logFolderPath = path.join(rootPath, 'logs')
        let osName = os.platform()
        if (osName === 'win32') {
            let appPath = process.env['LOCALAPPDATA']
            if (typeof appPath == "undefined") {
                //xp
                appPath = process.env['APPDATA']
            }
            //change root path
            rootPath = path.join(appPath, 'minicloud')
            logFolderPath = path.join(rootPath, 'logs')
        }
    }
    /**
...
let __getAppConfig = function*() {
        ...
        for (let i = 0; i < config.disk.length; i++) {
            ...
            if (!__exists(savePath)) {
                mkdirp.sync(savePath)
            }
            //create temp folder path
            let tempPath = path.join(savePath, 'minicloud', 'upload_temp')
            if (!__exists(tempPath)) {
        ...
        if (!config.convert_data_path) {
            config.convert_data_path = path.join(firstDiskPath, 'cache', 'convert_data')
        }
        if (!__exists(config.convert_data_path)) {
            mkdirp.sync(config.convert_data_path)
        }
        //create app temp folder
        if (!config.temp_path) {
            config.temp_path = path.join(firstDiskPath, 'cache', 'temp')
        }
        if (!__exists(config.temp_path)) {
